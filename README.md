...
如果手头是纯文本题库，用 `转换题库.py` 直接生成 `questions.js`，不用手敲 JSON：

```bash
python 转换题库.py 题库数据.txt              # 输出到当前目录的 questions.js
python 转换题库.py 题库数据.txt -o out.js    # 指定输出路径
```

源文件格式是这样，每题由一行题干加若干行选项组成，题与题之间空行隔开：
...
题干行是 `题号|答案|题型|题干`。几个宽松的地方：

- 答案大小写都认，顺序无所谓（`abd` 和 `ABD` 等价）
- 选项分隔符 `.` 和 `、` 都认
- 选项字母不限于 ABCD，A–H 都支持
- `解析：` 或 `解析|` 开头的行会被收进 `explain` 字段，没有就留空

转换前会做校验，**发现问题就报错退出、不写文件**：题号重复、答案超出选项范围、选项字母跳号、选项少于 2 个。题型和答案个数不匹配这类只是警告，不阻断。
...
├── 转换题库.py          # 把纯文本题库转成 questions.js（可选，Python 3.6+）
